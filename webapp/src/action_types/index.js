import {id} from '../manifest';

export default {
    RECEIVED_REVIEWS: `${id}_received_reviews`,
    RECEIVED_YOUR_PRS: `${id}_received_your_prs`,
    RECEIVED_YOUR_ASSIGNMENTS: `${id}_received_your_assignments`,
    RECEIVED_MENTIONS: `${id}_received_mentions`,
    RECEIVED_UNREADS: `${id}_received_unreads`,
    RECEIVED_CONNECTED: `${id}_received_connected`,
    RECEIVED_GITLAB_USER: `${id}_received_gitlab_user`,
    OPEN_ATTACH_NOTES_TO_ISSUE_MODAL: `${id}_open_attach_modal`,
    CLOSE_ATTACH_NOTES_TO_ISSUE_MODAL: `${id}_close_attach_modal`,
    RECEIVED_ATTACH_NOTES_RESULT: `${id}_receive_attach_notes`,
};
